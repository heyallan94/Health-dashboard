...
// Base simples seguindo sua referência:
// 60kg -> 150g, 70kg -> 200g, 80kg -> 250g.
...
const avaliarCarboTreino = (carboAtual, peso) => {
  ...
  if (pct < 0.4) {
    return {
      alvo,
      classe: "baixo",
      titulo: "Carbo baixo para treino",
      texto: `Você consumiu ${carboAtual}g de carbo. Para ${peso}kg, uma boa referência antes de treinar é perto de ${alvo}g no dia. Energia pode faltar.`,
    };
  }
  ...
  if (pct < 0.65) {
    return {
      alvo,
      classe: "baixo",
      titulo: "Estoque ainda baixo",
      texto: `Você está com ${carboAtual}g de carbo. Não está zerado, mas ainda está baixo para um treino forte. Referência: ${alvo}g.`,
    };
  }
  ...
  if (pct < 0.85) {
    return {
      alvo,
      classe: "medio",
      titulo: "Carbo ok, mas não ideal",
      texto: `Você tem ${carboAtual}g de carbo. Não está ruim, mas ainda não está no melhor ponto para treinar pesado. Alvo estimado: ${alvo}g.`,
    };
  }
  ...
  if (pct <= 1.2) {
    return {
      alvo,
      classe: "bom",
      titulo: "Bom nível para treino",
      texto: `Você está com ${carboAtual}g de carbo. Para ${peso}kg, isso está numa faixa boa para render no treino. Alvo estimado: ${alvo}g.`,
    };
  }
  ...
  return {
    ...
    classe: "alto",
    titulo: "Carbo alto",
    texto: `Você já passou bastante do alvo estimado de ${alvo}g. Dá energia, mas cuidado para não virar excesso calórico no dia.`,
  };
};
...
function Home() {
  ...
  useEffect(() => {
    let startK = kcalAnimada;
    let startP = proteinaAnimada;
    let startC = carboAnimado;

    const steps = 40;
    const stepK = (atualKcal - startK) / steps;
    const stepP = (atualProt - startP) / steps;
    const stepC = (atualCarbo - startC) / steps;
    ...
    const interval = setInterval(() => {
      ...
      setKcalAnimada(Math.round(startK));
      setProteinaAnimada(Math.round(startP));
      setCarboAnimado(Math.round(startC));

      if (count >= steps) {
        setKcalAnimada(atualKcal);
        setProteinaAnimada(atualProt);
        setCarboAnimado(atualCarbo);
  ...
  useEffect(() => {
    let start = aguaAnimada;
    const steps = 40;
    const step = (atualAgua - start) / steps;
    let count = 0;
    ...
    const interval = setInterval(() => {
      ...
      setAguaAnimada(Math.round(start));

      if (count >= steps) {
        setAguaAnimada(atualAgua);
        clearInterval(interval);
      }
    }, 2000 / steps);
  ...
  useEffect(() => {
    ...
    const atualA =
      localStorage.getItem(`aguaConsumida_${dataHoje()}`) ||
      localStorage.getItem("aguaConsumida");
  ...
  useEffect(() => {
    ...
    setPlanoCriado(!!plano);

    if (plano?.peso) setPesoUsuario(Number(plano.peso));
    if (plano?.carbo || plano?.metaCarbo) {
      setMetaCarbo(Number(plano.carbo || plano.metaCarbo));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const buscarAguaRemota = async () => {
      ...
      if (!error && data) {
        ...
        if (meta > 0) {
          localStorage.setItem("metaAgua", String(meta));
        }
      }
    };
                ...
                <div className="circulosWrapper">
                  <div>
                    <ProgressCircle
                      value={kcalAnimada}
                      max={metaKcal}
                      color={getColor(atualKcal, metaKcal)}
                    />
                  ...
                  <div>
                    <ProgressCircle
                      value={proteinaAnimada}
                      max={metaProteina}
                      color={getColor(atualProt, metaProteina)}
                    />
                  ...
                  <div>
                    <ProgressCircle
                      value={aguaLitros}
                      max={metaAguaLitros || 2}
                      color={getColorAgua(atualAgua, metaAgua)}
                    />
                  ...
                  <div>
                    <ProgressCircle
                      value={carboAnimado}
                      max={metaCarbo}
                      color={getColorCarbo(atualCarbo, metaCarbo)}
                    />
